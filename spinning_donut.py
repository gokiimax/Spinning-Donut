...
while run: 
    ...
    b = [' '] * screen_size # fills empty space
    ...
    for j in range(0, 628, theta_spacing): # from 0 to 2pi
        for i in range(0, 628, phi_spacing): # from 0 to 2pi
            ...
            n = math.sin(B)
            t = c * h * g - f * e
            x = int(x_offset + 40 * D * (l * h * m - t * n)) # 3D x coordinate after rotating
            y = int(y_offset + 20 * D * (l * h * n + t * m)) # 3D y coordinate after rotating
            o = int(x + columns * y) # 3D z coordinate after rotating
